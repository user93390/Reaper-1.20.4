...
public class PopCounter extends ReaperModule {
    ...
    private void onEntityRemoved(EntityRemovedEvent event) {
        if (event.entity instanceof PlayerEntity player) {
            String n = player.getName().toString();
            UUID u = player.getUuid();
    ...
    private String getPopAlert(PlayerEntity p, int pops, boolean died) {
        String popAlert;
        if (died) popAlert = p.getName().toString() + " died after popping " + pops + getPopGrammar(pops);
        else popAlert = p.getName().toString() + " popped " + pops + getPopGrammar(pops);
        return popAlert;
    }
