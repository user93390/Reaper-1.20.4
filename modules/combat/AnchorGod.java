...
public class AnchorGod extends ReaperModule {
    ...
    private void breakAnchor(AnchorPlacement placement) {
        ...
        if (!strictInv.get() || !strictMotion.get()) Interactions.setSlot(anchor.slot(), false); // swap to anchors rather than the "old slot" because we need to be holding anchors to place again anyway
    }
